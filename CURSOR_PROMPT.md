...
- **Display format**: Dates are always displayed as `dd/mmm/yyyy` (e.g., `15/Jan/2026`) regardless of configuration
- **History order**: Date history is displayed in reverse chronological order (newest first, oldest last)
- **Current date exclusion**: Current date must NOT appear in struck-out history to prevent duplicate display
- The `date_format` field in config is for documentation purposes only; display is always `dd/mmm/yyyy`
